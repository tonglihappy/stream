...
func (f *CFrame) IsHeader() bool {
	return len(f.Body) > 0 && f.Body[1] == 0
}

func (f *CFrame) IsKeyFrame() bool {
	return f.Flavor == KEYFRAME
}
...
func (f MetaFrame) String() string {
	...
	if err == nil {
		...
		case amf0.StringType("onHaveIndex"):
			md, err := dec.Decode()
			if err == nil {
...
func NewReader(inFile io.Reader) *FlvReader {
	return &FlvReader{
		InFile: inFile,
		width:  0,
		height: 0,
...
func (frReader *FlvReader) readFrame() (*CFrame, error) {
	...
	n, err = io.ReadFull(frReader.InFile, tagHeaderB)
	if n == 0 {
		return nil, err
	}
	...
	//fmt.Println(tagType, bodyLen, ts, tsExt, stream)
	var dts uint32
	dts = (tsExt << 24) | ts

	bodyBuf := make([]byte, bodyLen)
	n, err = io.ReadFull(frReader.InFile, bodyBuf)
	if err != nil {
		return nil, err
	}
	...
	n, err = io.ReadFull(frReader.InFile, prevTagSizeB)
	if err != nil {
		return nil, err
	}
	...
	//fmt.Println(prevTagSize, bodyLen+uint32(TAG_HEADER_LENGTH))

	if prevTagSize != 0 && prevTagSize != bodyLen+uint32(TAG_HEADER_LENGTH) {
		return nil, nil
	}
	return pFrame, nil
}

func isHeader(body []byte) bool {
	return body[1] == 0
}

...
Stream::Stream(const char *file, const char *url){
...
int Stream::open_input_file(const char* filename){
    ...
    if (!ifmt_ctx) {
        m_err = "ifmt_ctx init failed";
    }
    
    if((ret = avformat_open_input(&ifmt_ctx, filename, 0, 0)) < 0) {  
        m_err = std::string("Could not open input file: ") +  filename;
        return ret;
    }  

    if ((ret = avformat_find_stream_info(ifmt_ctx, 0)) < 0) {  
        m_err =  "Failed to retrieve input stream information"; 
        return ret; 
    }  
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (!ist->dec_ctx) {
            m_err = "Error allocating the decoder context.";
            return -1;
        }
        ret = avcodec_parameters_to_context(ist->dec_ctx, par);
        if (ret < 0) {
            m_err = "Error initializing the decoder context.";
            return -1;
        }
...
int Stream::open_output_file(const char* filename){
    ...
    if (!ofmt_ctx) {  
        m_err = std::string("init ofmt failed! ") + filename;
        ret = AVERROR_UNKNOWN;  
        return ret;  
    }
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) { 
        ...
        if (!out_stream) {  
            m_err = "Failed allocating output stream";
            ret = AVERROR_UNKNOWN;  
            return ret;
        }  
        ...
        if (ret < 0) {  
            m_err = "Failed to copy context from input to output stream codec context";
            return ret;  
        }  
    }
    ...
    if (!(ofmt->flags & AVFMT_NOFILE)) {  
        ret = avio_open2(&ofmt_ctx->pb, filename, AVIO_FLAG_WRITE, &ofmt_ctx->interrupt_callback,
            NULL);
        if (ret < 0) {  
            m_err = std::string("Could not open output ") + filename;
            return ret; 
        }  
    }
...
int Stream::get_written_pkt_cnt() {
    int cnt = 0;

    for (int i = 0; i < nb_input_streams; i++) {
        OutputStream *ost = output_streams[i];
        cnt += ost->packets_written;
    }
    ...
    while(!m_exit) {  
        ...
        if (ist->nb_packets == 1){
            for (i = 0; i < ist->st->nb_side_data; i++) {
                ...
                if (!dst_data) {
                    m_err = "av_packet_new_side_data error!";
                    goto finish;
                }
        ...
        if (ret < 0) {
            m_err = "process input pakcers failed!";
            break;
        }
        av_packet_unref(&pkt);
    }
...
int create_stream(const char* filename, const char* url){
    ...
    std::cout << "exit flag: " << recv_exit << std::endl;
    std::cout << "delete stream success: " << url << std::endl;
    delete stream;
    stream = NULL;
    recv_exit = 0;
    return ret; 
...
{  
    ...
    avformat_network_init(); 
    */
    Stream *stream = new Stream(NULL, NULL);
